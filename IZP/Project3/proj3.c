 ...
 */
inline void printMsg(ErrCode code)
{
...
int loadMatrix(FILE *soubor, TMatrix *matice)
{
  ...
  for (unsigned  int r = 0; r < matice->rows; r++) 
  { 
    for (unsigned  int s = 0; s < matice->cols; s++) 
    {
...
{ 
  ...
  // výpoèet a ulozeni vysledku do prvni matice
  for (unsigned  int r = 0; r < add->rows; r++)
  {
    for (unsigned int s = 0; s < add->cols; s++)
    {
...
{ 
  ...
  {
    for (unsigned  int s = 0; s < dest->cols; s++)
    {
      dest->matrix[r][s] = 0; // vynulovani prvku pole  
      for (unsigned  int t = 0; t < mult1->cols; t++)
...
int monoMatrix(const TMatrix *source) 
{
  ...
  {
    for (unsigned int s = 0; s < source->cols; s++)
    {
...
int spiralMatrix(const TMatrix *source) 
{
  ...
  // vypocet    
  for (int i = 1; i < pocetCyklu+1; i++)
  {
    for (int j = 0; j < pocetKroku; j++) 
    {
